...
class TestFilterZeroAdvBatch(unittest.TestCase):
    ...
    @parameterized.expand(
        (
            ...
            ("4nz_12z_dp4", 4, 12, 8, 4, 4, 0, 4, 0.25, 0.25, 24.0),
            ("1nz_15z_dp4", 1, 15, 4, 4, 4, 3, 4, 0.25, 0.25, 12.0),
        )
    )
    def test_filtered_mixed(
        ...
        self.assertAlmostEqual(
            filtered.meta_info[KEY_AVG_TOKENS_ZERO_ADV_PER_DP_GROUP], expected_avg_tokens_zero_adv, places=6
        )

    @parameterized.expand(
        (
            # (name, bs, seq, dp, kept, orig_bs_per_dp, seq_corr, token_corr, avg_tokens_zero_adv)
            ("16x8_dp4", 16, 8, 4, 4, 4, 0.25, 0.25, 24.0),
            ("8x4_dp2", 8, 4, 2, 2, 4, 0.25, 0.25, 12.0),
            ("32x4_dp8", 32, 4, 8, 8, 4, 0.25, 0.25, 12.0),
        )
    )
